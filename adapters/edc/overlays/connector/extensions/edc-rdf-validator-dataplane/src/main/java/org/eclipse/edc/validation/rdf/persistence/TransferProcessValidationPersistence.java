package org.eclipse.edc.validation.rdf.persistence;
...
import org.eclipse.edc.validation.rdf.model.ValidationPersistenceKeys;
 ...
 * keys with the {@code edc.rdf.validation.*} prefix using a raw JSONB merge update.
 * <p>
...
public class TransferProcessValidationPersistence {
     ...
     * Only keys with the {@code edc.rdf.validation.*} prefix are written.
     *
                ...
                try (var rs = ps.executeQuery()) {
                    ...
                    for (var key : obj.keySet()) {
                        if (!key.startsWith(ValidationPersistenceKeys.KEY_PREFIX)
                                && !key.startsWith(ValidationPersistenceKeys.LEGACY_KEY_PREFIX)) {
                            continue;
                        }
                        var normalizedKey = key.startsWith(ValidationPersistenceKeys.LEGACY_KEY_PREFIX)
                                ? ValidationPersistenceKeys.KEY_PREFIX + key.substring(ValidationPersistenceKeys.LEGACY_KEY_PREFIX.length())
                                : key;
                        var v = obj.get(key);
                        ...
                        if (v.getValueType() == JsonValue.ValueType.STRING) {
                            out.put(normalizedKey, ((JsonString) v).getString());
                        } else {
                            out.put(normalizedKey, v.toString());
                        }
                    }
                    return out;
                }
