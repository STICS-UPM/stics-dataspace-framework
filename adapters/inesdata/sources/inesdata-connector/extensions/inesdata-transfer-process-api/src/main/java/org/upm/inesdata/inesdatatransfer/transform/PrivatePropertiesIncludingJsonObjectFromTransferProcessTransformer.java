package org.upm.inesdata.inesdatatransfer.transform;
 ...
 * that bypasses the in-memory map, so we merge DB-backed {@code inesdata.rdf.validation.*} keys here.
 * <p>
...
public class PrivatePropertiesIncludingJsonObjectFromTransferProcessTransformer extends AbstractJsonLdTransformer<TransferProcess, JsonObject> {
    ...
    /**
     * Promote plain keys (e.g. {@code inesdata.rdf.validation.status}) to absolute IRIs so they survive
