...
from validation.components.semantic_virtualization.gtfs_bench_official import (
    ...
    resolve_gtfs_bench_source_dir,
)


COMPONENT_KEY = "semantic-virtualization"
SUITE_NAME = "gtfs-bench-official-dataset"
...
def _build_runtime_selection(dump_path: Path) -> dict[str, list[dict[str, str | None]]]:
    ...
    if len(selected_trips) != len(selected_trip_ids):
        ...
        raise RuntimeError(f"Could not build GTFS runtime sample; missing expected trips: {missing}")
    ...
    if parent_station_ids:
        ...
        selected_stops.extend(
            row
            for row in _select_first_matching(stops, "stop_id", parent_station_ids)
            if row["stop_id"] not in known_stop_ids
        )
    ...
    if empty_tables:
        raise RuntimeError(f"Could not build GTFS runtime sample; empty selected tables: {', '.join(empty_tables)}")
    return selected


def build_gtfs_bench_official_sample(
    ...
    selected = _build_runtime_selection(dump_path)
    commit = _git_output(resolved_source_dir, ["rev-parse", "HEAD"])
    remote = _git_output(resolved_source_dir, ["config", "--get", "remote.origin.url"])
    required_paths = {
        "dump": dump_path,
        "mapping": resolved_source_dir / "mappings" / "gtfs-csv.rml.ttl",
        "ontology": resolved_source_dir / "ontology" / "gtfs.ttl",
        "simple_query_q1": resolved_source_dir / "queries" / "simple" / "q1.rq",
        "full_query_q1": resolved_source_dir / "queries" / "q1.rq",
    }
    return {
        "dataset_name": "GTFS-Madrid-Bench",
        "sample_name": "GTFS-Bench official runtime sample",
        "domain": "mobility",
        ...
        "missing_required_paths": [str(path) for path in required_paths.values() if not path.is_file()],
        "selection": {
            "objective": "Runtime sample selected from the synchronized GTFS-Madrid-Bench source repository.",
            ...
            "notes": [
                "The repository source remains the authoritative dataset.",
                "The selected rows are derived during validation and are not versioned as a dataset fixture.",
            ],
        },
        "tables": selected,
    }
...
def write_gtfs_bench_runtime_sample_csvs(sample: dict[str, Any], output_dir: str | os.PathLike[str]) -> dict[str, str]:
    output_root = Path(output_dir)
    output_root.mkdir(parents=True, exist_ok=True)
    tables = sample.get("tables") or {}
    written: dict[str, str] = {}
    for table, headers in GTFS_HEADERS.items():
        csv_path = output_root / f"{table}.csv"
        _write_csv(csv_path, headers, list(tables.get(table) or []))
        written[table] = str(csv_path)
    return written


def validate_gtfs_bench_official_dataset_sample(
    source_dir: str | os.PathLike[str] | None = None,
) -> dict[str, Any]:
    assertions: list[str] = []
    try:
        ...
        return {
            ...
            "assertions": [str(exc)],
            "dataset_name": "GTFS-Madrid-Bench",
            "source_dir": str(resolve_gtfs_bench_source_dir(source_dir)),
        }

    assertions.extend(
        f"Required GTFS-Bench source file is missing: {path}" for path in sample.get("missing_required_paths") or []
    )
    tables = sample.get("tables") or {}
    agencies = tables.get("AGENCY") or []
    calendars = tables.get("CALENDAR") or []
    routes = tables.get("ROUTES") or []
    shapes = tables.get("SHAPES") or []
    stops = tables.get("STOPS") or []
    stop_times = tables.get("STOP_TIMES") or []
    trips = tables.get("TRIPS") or []
    ...
    for row in routes:
        if row["agency_id"] not in agency_ids:
            assertions.append(f"ROUTES references missing agency_id {row['agency_id']}")
    for row in trips:
        if row["route_id"] not in route_ids:
            assertions.append(f"TRIPS references missing route_id {row['route_id']}")
        if row["service_id"] not in service_ids:
            assertions.append(f"TRIPS references missing service_id {row['service_id']}")
        if row["shape_id"] not in shape_ids:
            assertions.append(f"TRIPS references missing shape_id {row['shape_id']}")
    for row in stop_times:
        if row["trip_id"] not in trip_ids:
            assertions.append(f"STOP_TIMES references missing trip_id {row['trip_id']}")
        if row["stop_id"] not in stop_ids:
            assertions.append(f"STOP_TIMES references missing stop_id {row['stop_id']}")
    for row in stops:
        parent_station = row.get("parent_station") or ""
        if parent_station and parent_station not in stop_ids:
            assertions.append(f"STOPS references missing parent_station {parent_station}")
...
def run_gtfs_bench_official_dataset_validation(
    experiment_dir: str | os.PathLike[str] | None = None,
    *,
    source_dir: str | os.PathLike[str] | None = None,
) -> dict[str, Any]:
    started_at = datetime.now()
    validation = validate_gtfs_bench_official_dataset_sample(source_dir)
    ...
    case = {
        ...
        "description": "Validate a runtime sample derived from the official GTFS-Madrid-Bench source repository",
        "type": "support",
        "case_group": "support",
        "validation_type": "support",
        "dataspace_dimension": "official_dataset_traceability",
        "mapping_status": "mapped",
        "automation_mode": "offline_source_sample",
        "execution_mode": "offline_source_sample",
        ...
        "evidence": validation,
        "expected_result": (
            "The framework can derive a deterministic validation sample from the synchronized "
            "GTFS-Madrid-Bench source repository without versioning a reduced dataset fixture."
        ),
    }
    summary = {
        "total": 1,
    ...
    if component_dir:
        report_path = component_dir / "semantic_virtualization_gtfs_bench_official_dataset.json"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate a GTFS-Bench runtime sample derived from the official source repository",
    )
    ...
    report = run_gtfs_bench_official_dataset_validation(
