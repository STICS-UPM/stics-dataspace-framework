...
from validation.components.semantic_virtualization.gtfs_bench_materialization import (
    ...
    materialize_gtfs_bench_official_graph,
...
class SemanticVirtualizationGtfsBenchMaterializationTests(unittest.TestCase):
    def test_official_csv_mapping_can_be_adapted_to_runtime_sample(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = adapt_official_csv_mapping(
                source_dir=source_dir,
                sample_csv_dir=report["artifacts"]["runtime_sample_csv_dir"],
            )
        ...
        self.assertEqual(result["source_rewrites"]["/data/SHAPES.csv"], "runtime-sample-csv/SHAPES.csv")
        self.assertTrue(result["adapted_mapping_sha256"])

    def test_runtime_sample_materializes_queryable_gtfs_graph(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            source_dir = create_gtfs_source(tmpdir)
            graph, summary = materialize_gtfs_bench_official_graph(source_dir=source_dir)
    ...
    def test_materialization_validation_runs_official_q1_queries(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            source_dir = create_gtfs_source(tmpdir)
            result = validate_gtfs_bench_official_materialization(source_dir=source_dir)
